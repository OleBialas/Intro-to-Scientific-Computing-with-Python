...
:::{#exr-}
Select the data from the first EEG channel.
:::
...
:::{#exr-}
Select the from the first half of all time points for EEG channels 10 to 20.
:::
...
Filtering can also be used to used to link to different arrays. In this section we are going to load an array that contains the time points for the epochs from the previous array. We can use that array to define a mask and then apply that mask to the array of epoch. 
Numpy also allow us to combine multiple masks using the and/or operators `&` and `|`.
