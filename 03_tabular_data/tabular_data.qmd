...
Most of pandas functionalities come as so called methods of the data frame object.
...
Once we loaded the data into a data frame, we can access a given column by providing its name as a string inside square brackets.
...
Just like arrays, a data frame can be filtered using Boolean masks.
...
This is very useful for statistical analysis.
For example, we could check whether the average age of passengers differs between sexes by extracting the values where `df["sex"]=="male"` and `df["sex"]=="female"` and computing their respective means.
...
:::{#exr-}
How are the passengers in `"pclass"` 1 distributed across the `"embark_town"`s compared to those in `"pclass"` 3?
:::
...
:::{#exr-}
Create a bar plot for the same data by specifying `kind="bar"`.
:::
...
:::{#exr-}
Perform an analysis of variance (ANOVA) to compare the difference in the dependent variable `"fare"` between `"embark_town"`s. 
:::
...
:::{#exr-}
Calculate the pairwise correlations for all columns in `df`. Assign the data frame returned by `pg.pariwise_corr` to a new variable called `df_corr` and print the columns `"X"`, `"Y"` and `"r"`
:::
