...
To explore this, we are going to visualize changes in fertility rates over time in different countries.
Per default, Matplotlib will show plots immediately after executing a cell.
...
:::{#exr-}
Plot the fertility rates (`"fertility"`) across time (`"year"`) for Germany (`df_ger`) and the United States (`df_usa`). Add a `label` to each line and add a `plt.legend()`
:::
...
| `plt.savefig("myfig.png", dpi=300)` | Store the current figure as `"myfile.png"` with a resolution of 300 `dpi` (dots per inch) |
...
Visualizations are a great for understanding the relationship between variables.
...
Matplotlib takes an **imperative** approach to visualization where we specific exactly what we want to draw.
This provides fine-grained control but also requires writing a lot of code.
On the contrary, Seaborn takes a more **declarative** approach where we specify what data we want to visualize.
...
Then, create the scatter plot from @exr-sns_scatter and draw it to the subplot by using the `ax` argument of `sns.scatter()`.
:::
...
Then, create the scatter plot from @exr-sns_scatter and the kde plot from @exr-sns_kde and draw them to the subplots `ax1` and `ax2` by using the `ax` argument of `sns.scatterplot` and `sns.kdeplot`.
:::
